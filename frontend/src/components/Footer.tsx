...
export const Footer = () => {
  ...
  return (
    <footer className='bg-gray-100 border-y-1 text-sm mt-1'>
      <div className='container mx-auto 2xl:px-24 my-7'>
        {/* Link */}
        <div className='nav-link text-center md:text-right my-4'>
          <Link to='/about/'
            className={`mx-2 sm:mx-6 ${isCurrent('/about/') ? 'border-b-2' : 'hover:border-b-2'}`}
            aria-current={isCurrent('/about/') ? 'page' : undefined}>About Us |</Link>
          <Link to='/contact/'
            className={`mx-2 sm:mx-6 ${isCurrent('/contact/') ? 'border-b-2' : 'hover:border-b-2'}`}
            aria-current={isCurrent('/contact/') ? 'page' : undefined}>Contact |</Link>
          <Link to='/faq/'
            className={`mx-2 sm:mx-6 ${isCurrent('/faq/') ? 'border-b-2' : 'hover:border-b-2'}`}
            aria-current={isCurrent('/faq/') ? 'page' : undefined}>FAQ |</Link>
          <Link to='/privacy/'
            className={`mx-2 sm:mx-6 ${isCurrent('/privacy/') ? 'border-b-2' : 'hover:border-b-2'}`}
            aria-current={isCurrent('/privacy/') ? 'page' : undefined}>Privacy |</Link>
          <Link to='/terms/'
            className={`mx-2 sm:mx-6 ${isCurrent('/terms/') ? 'border-b-2' : 'hover:border-b-2'}`}
