      ...
      <nav className="container mx-auto font-medium">
         ...
         <div className="border-y-2 border-gray-500 py-1 mb-2">
            <div className="nav-link flex justify-between overflow-x-auto px-3 text-xl">
               <Link to="/"
                  className={`px-3 py-2 ${pathname === "/" ? "border-b-2" : "hover:border-b-2"}`}
                  aria-current={pathname === "/" ? "page" : undefined}>News</Link>
               <Link to="/activity/"
                  className={`px-3 py-2 ${isActive("/activity/") ? "border-b-2" : "hover:border-b-2"}`}
                  aria-current={isActive("/activity/") ? "page" : undefined}>Activity</Link>
               <Link to="/collection/"
                  className={`px-3 py-2 ${isActive("/collection/") ? "border-b-2" : "hover:border-b-2"}`}
                  aria-current={isActive("/collection/") ? "page" : undefined}>Collection</Link>
               <Link to="/kenshi/"
                  className={`px-3 py-2 ${isActive("/kenshi/") ? "border-b-2" : "hover:border-b-2"}`}
                  aria-current={isActive("/kenshi/") ? "page" : undefined}>Kenshi</Link>
               <Link to={memberHref}
                  className={`px-3 py-2 ${isActive("/member/") ? "border-b-2" : "hover:border-b-2"}`}
